    ...
    do
        ...
        if [ -f "$f" ]; then
            ...
            if [ "$DUMP_FLAG" == "1" ] ; then
                ...
                nice -n 19 tar -zcvf $tgzFile $dumpName $VERSION_FILE $stbLogFile $ocapLogFile $messagesTxtFile $appStatusLogFile $appLogFile $CORE_LOG 2>&1 | logStdout
                if [ $? -eq 0 ]; then
                    logMessage "Success Compressing the files, $tgzFile $dumpName $VERSION_FILE $stbLogFile $ocapLogFile $messagesTxtFile $appStatusLogFile $appLogFile $CORE_LOG "
                else
                   logMessage "Compression Failed ."
                fi
            fi

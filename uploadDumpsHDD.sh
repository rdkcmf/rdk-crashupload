...
cleanup()
{
    ...
    if [ ! -f /opt/.upload_on_startup ];then
        ...
        if [ ! -f "$ON_STARTUP_DUMPS_CLEANED_UP" ] ; then
            ...
            if [ "$DUMP_FLAG" = "1" ] ; then
                deleteAllButTheMostRecentFile "$COREFILES_BACKUP_DIR"
            fi
...
sigkill_function()
{
    echo "Systemd Killing the process, Removing the script locks"
    [ -f "$crashLoopFlagFile" ] && rm -f "$crashLoopFlagFile"
    remove_lock $LOCK_DIR_PREFIX
    remove_lock "$TIMESTAMP_FILENAME"
}
...
sigterm_function()
{
    echo "Systemd Terminating the process, Removing the script locks"
    [ -f "$crashLoopFlagFile" ] && rm -f "$crashLoopFlagFile"
    remove_lock $LOCK_DIR_PREFIX
    remove_lock "$TIMESTAMP_FILENAME"
}
...
if [ "$DUMP_FLAG" = "1" ] ; then
    ...
    CRASH_PORTAL_PATH="$PERSISTENT_PATH/crashportal_uploads/minidumps/"
    #to limit this to only one instance at any time..
...
failOverUploadToCrashPortal()
{
    ...
    if [ "$DEVICE_TYPE" = "broadband" ]; then
        ...
        if [ $UseCodebig -eq 1 ]; then
            logMessage "failOverUploadToCrashPortal: Codebig is enabled UseCodebig=$UseCodebig"
            if [ "$DEVICE_TYPE" = "mediaclient" ]; then
                ...
                if [ $skipcodebig -eq 0 ]; then
                    ...
                    do
                        logMessage "failOverUploadToCrashPortal: Attempting Codebig core upload"
                        codebigUpload $url
                        ret=$TLSRet
                        http_code=$(awk -F\" '{print $1}' $HTTP_CODE)
                        if [ "$http_code" = "200" ]; then
                           logMessage "failOverUploadToCrashPortal: Codebig core upload success return:$ret, httpcode:$http_code"
                           IsDirectBlocked
                           skipDirect=$?
                           if [ $skipDirect -eq 0 ]; then
                               UseCodebig=0
                ...
                if [ "$http_code" = "000" ];then
                    ...
                    if [ $skipdirect -eq 0 ]; then
                        UseCodebig=0
                        logMessage "failOverUploadToCrashPortal: Codebig core upload failed with httpcode:$http_code, attempting direct"
                        if [ -f $EnableOCSPStapling ] || [ -f $EnableOCSP ]; then
                           logMessage "Upload string: curl -v $TLS $INTERFACE_OPTION --cert-status -w '%{http_code}\n' --upload-file ./$coreFile https://${host}:8090/upload?filename=$remotePath/$dirnum/$coreFile&user=ccpstbscp"
                           curl -v $TLS $INTERFACE_OPTION -w '%{http_code}\n' --upload-file ./$coreFile "https://${host}:8090/upload?filename=$remotePath/$dirnum/$coreFile&user=ccpstbscp" --cert-status --connect-timeout $CURL_UPLOAD_TIMEOUT > $HTTP_CODE
                        else
            ...
            logMessage "failOverUploadToCrashPortal: Codebig is disabled: UseCodebig=$UseCodebig"
            IsDirectBlocked
            skipdirect=$?
            if [ $skipdirect -eq 0 ]; then
            ...
            if [ "$http_code" = "000" ]; then
                if [ "$DEVICE_TYPE" = "mediaclient"]; then
                    logMessage "failOverUploadToCrashPortal: Direct core upload failed with httpcode:$http_code, attempting Codebig"
                    IsCodeBigBlocked
                    skipcodebig=$?
                    if [ $skipcodebig -eq 0 ]; then
                        while [ $cbretries -le $CB_MAX_UPLOAD_ATTEMPTS ]
...
if [ ! -d $WORKING_DIR ]; then mkdir -p $WORKING_DIR; exit 0; fi
if [ ! -d $COREFILES_BACKUP_DIR ]; then mkdir -p $COREFILES_BACKUP_DIR; fi
cd $WORKING_DIR
...
processDumps()
{
    ...
    do
        ...
        if [ -f "$f" ]; then
            ...
            # backup only corefiles
            if [ "$DUMP_FLAG" == "1" ]; then
                # removing timestamp in corefiles and copy them into backup directory.
                cp $f $COREFILES_BACKUP_DIR/core.prog${f##*core.prog}
            fi
		...
		if [ "$SEC_DUMP" = "true" ]; then
            ...
            # skipping archiving coredumps for PROD build
            if [ `shouldProcessFile $f` = 'false' ]; then
                logMessage "Skip archiving and delete $f as we are trying to archive coredump for a PROD build and it is not mpeos or xdiscovery."
                rm -f $f
                continue
            fi
